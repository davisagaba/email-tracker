...
function processIncomingMessage(parsed) {
  ...
  db.prepare(
    `INSERT INTO messages (contact_id, direction, subject, body, message_id, in_reply_to, read, tracking_id)
     VALUES (?, 'inbound', ?, ?, ?, ?, 0, ?)`
  ).run(contactId, parsed.subject || null, body, parsed.messageId || null, parsed.inReplyTo || null, trackingId || null);
